...
chrome.action.onClicked.addListener(async () => {
	...
	let response = await chrome.tabs.sendMessage(tab.id, { 'action': 'toggle' })
		.catch(() => null)

	if (!response) {
		await chrome.scripting.executeScript({ files: [ 'ctre_content.js' ], target: { tabId: tab.id} })
		await chrome.tabs.sendMessage(tab.id, { 'action': 'toggle' })
			.catch(() => { /* this is likely a protected tab (chrome://, etc.) - ignore */ })
	}
})
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
	...
	if (msg.action == 'get_saved_elms') {
		...
		idbKeyval.get('settings').then(data => sendResponse(data || '{}'))
		return IS_ASYNC
	} else if (msg.action == 'set_settings') {
		idbKeyval.set('settings', msg.data)
		return IS_ASYNC
	} else if (msg.action == 'get_hotkey') {
		chrome.commands.getAll((cmds) => {
			let shortcut = cmds && cmds[0] && cmds[0].shortcut
			sendResponse(shortcut || '')
		})
		return IS_ASYNC
	} else if (msg.action == 'goto_hotkey_settings') {
